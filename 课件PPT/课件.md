...
现在，我们在 `Mc入口 > 脚本入口 > plugins` 目录下，新建一个文件 **game_fightzombie.js**。
...
由于需要确保所有参与游戏的用户都处于生存模式(都成了神，还玩什么游戏)，因此我们再添加一个相应的函数来处理。
...
这就需要我们先设置好战斗位置。获取地点定位的方法如 **雪球大战** 所述，这里不再重复。
...
function goToBase(p, location) {
  if (p instanceof Array) {  // 如果传进来的是数组
    for (var i = 0; i < p.length; i++) {
      goFightBase(p[i], location);
    }
  } else if (typeof p === 'string') {  // 是玩家名称
    goFightBase(utils.player(p), location);
  } else if (p) {  // 是玩家对象
    p.teleport(location);
  }
}
...
function generateZombies() {
  ...
  generateZombiesTimer = setTimeout(generateZombies, 1500);
}
```
...
function start() {
  ...
  setTimeout(generateZombies, 1000);
...
function damageHandler(event) {
  ...
  // 如果攻击者不是本游戏的玩家那也什么都不做
  if (names.indexOf(damager.name) === -1) { return; }
  if (damagee.isPlayer()) {  // 如果受攻击者是玩家
    event.setCancelled(true);
    return;
  }

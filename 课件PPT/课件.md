...
6. 当玩家遭到雷击时，会掉 4 滴血，同时，将有 3 秒钟的奔跑极速时间。
7. 当玩家死亡后，所在队伍的 Dead 值会加 -1 。
8. 当距离结束 10 秒前，用倒计时通知所有玩家。
9. 当游戏结束后，用大标题通知玩家是否赢得比赛。
10. 重置所有变量，使得玩家们可以重新进行游戏。
...
```javascript
command('/jsp game_snowball_join',
...
```javascript
command('/jsp game_snowball_leave',
  function(parameters, player) {
...
```javascript
/* 给绿队着装 */
function dressTeamGreen() {
  var player = null;	// 声明玩家容器
  for (var i = 0; i < team_green.length; i++) {
    player = utils.player(team_green[i]);
    player.setOp(true);
    slash([
      'replaceitem entity ' + player.name 
...
```javascript
/* 给金队着装 */
function dressTeamGold() {
  var player = null;	// 声明玩家容器
  for (var i = 0; i < team_gold.length; i++) {
    player = utils.player(team_gold[i]);
    player.setOp(true);
    slash([
      'replaceitem entity ' + player.name 
...
细看不难发现，这个函数竟然自己调用了自己！因此，这里又新增了一个知识点——**递归调用**。
